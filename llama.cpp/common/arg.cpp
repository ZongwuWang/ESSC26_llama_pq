...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--pq-decode"},
        "use PQ (product quantization) GEMV kernels for decode-time linear layers",
        [](common_params & params) {
            params.pq_decode = true;
        }
    ));
    add_opt(common_arg(
        {"--pq-ds"},
        "N",
        string_format("PQ d_sub: sub-vector dims (default: %d)", params.pq_ds),
        [](common_params & params, int value) {
            params.pq_ds = value;
        }
    ));
    add_opt(common_arg(
        {"--pq-mode"},
        "auto|s1|s2",
        string_format("PQ partition mode: auto = S1(input-dim) for qkv/gate/up + S2(output-dim) for down_proj (default: auto)"),
        [](common_params & params, const std::string & value) {
            if (value == "s1") params.pq_mode = 1;
            else if (value == "s2") params.pq_mode = 2;
            else params.pq_mode = 0;
        }
    ));
    add_opt(common_arg(
        {"--repack"},
