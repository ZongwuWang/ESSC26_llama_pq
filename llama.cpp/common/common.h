...
struct common_params {
    ...
    // PQ decode-time GEMV (pq_cpu kernels): PQ-quantize linear weights at
    // load, use PQ GEMV only during token generation (n_tokens == 1)
    bool pq_decode         = false; // enable PQ decode GEMV
    int  pq_ds             = 2;     // d_sub (sub-vector dims)
    int  pq_mode           = 0;     // 0=auto, 1=S1 (input-dim), 2=S2 (output-dim)
