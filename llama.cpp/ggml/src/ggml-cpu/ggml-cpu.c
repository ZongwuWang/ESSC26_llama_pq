...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    // PQ decode-time GEMV: registered linear weights, single-row (decode)
    // only. Must run before the extra-buffer (repack/AMX) intercept, which
    // otherwise claims F16-weight mul_mat.
    if (tensor->op == GGML_OP_MUL_MAT && ggml_pq_enabled()) {
        ...
        if (tensor->type == GGML_TYPE_F32 &&
            ...
            tensor->nb[1] == tensor->ne[0]*sizeof(float) &&
            (src1->type == GGML_TYPE_F32 || src1->type == GGML_TYPE_F16)) {
            if (ggml_pq_mul_mat_fused(src0->name, src1, src1->data, src1->type,
                                      (float *)tensor->data, src0->ne[0], src0->ne[1],
                                      params->ith, params->nth, params->threadpool) !=
                GGML_PQ_MM_NONE) {
                return;
            }
        }
...
// ---------------------------------------------------------------------------
// Optional per-op node timing (GGML_NODE_TIMING=1): aggregates thread-0 busy
// time and the following barrier wait (stragglers) per op type, printing a
// summary every GGML_NODE_TIMING_GRAPHS graphs.
// ---------------------------------------------------------------------------
struct ggml_op_acc {
    uint64_t count;
    uint64_t busy_us;    // thread-0 compute span
    uint64_t barrier_us; // thread-0 wait at the barrier AFTER this node
};
static struct ggml_op_acc_state {
    uint64_t graphs;
...
static void ggml_node_timing_dump(void) {
    uint64_t busy = 0, n = 0;
    for (int op = 0; op < GGML_OP_COUNT; op++) {
        busy += g_node_timing.g_op_acc[op].busy_us;
        busy += g_node_timing.g_op_acc[op].barrier_us;
        n    += g_node_timing.g_op_acc[op].count;
    }
    fprintf(stderr, "NODE-TIMING: %llu graphs, %llu nodes, busy+straggler=%.1f ms/graph "
            "(barrier share=%.0f%%)\n",
            (unsigned long long)g_node_timing.graphs, (unsigned long long)n,
            busy / 1000.0 / (g_node_timing.graphs ? g_node_timing.graphs : 1),
            busy ? 100.0 * g_node_timing.barrier_total_us / busy : 0.0);
    // top ops by busy+barrier
    int idx[GGML_OP_COUNT];
    for (int i = 0; i < GGML_OP_COUNT; i++) idx[i] = i;
    for (int a = 1; a < GGML_OP_COUNT; a++)
        for (int b = a; b > 0; b--) {
            uint64_t va = g_node_timing.g_op_acc[idx[b-1]].busy_us + g_node_timing.g_op_acc[idx[b-1]].barrier_us;
            uint64_t vb = g_node_timing.g_op_acc[idx[b]].busy_us + g_node_timing.g_op_acc[idx[b]].barrier_us;
            if (vb > va) { int tmp = idx[b-1]; idx[b-1] = idx[b]; idx[b] = tmp; } else break;
        }
    for (int a = 0; a < 12; a++) {
        const int op = idx[a];
        const struct ggml_op_acc * acc = &g_node_timing.g_op_acc[op];
        if (!acc->count) continue;
        fprintf(stderr, "  %-22s n=%7llu  busy=%8.2f ms  barrier=%8.2f ms  (avg %.1f/%.1f us)\n",
                ggml_op_name_short((enum ggml_op)op),
                (unsigned long long)acc->count,
                acc->busy_us / 1000.0, acc->barrier_us / 1000.0,
                acc->busy_us / (double)acc->count, acc->barrier_us / (double)acc->count);
    }
    ggml_pq_timing_dump_mulmat();
    memset(&g_node_timing, 0, sizeof(g_node_timing));
}
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        // PQ decode fusion: flush a pending same-input GEMV group before this
        // node computes (its inputs may read the group's outputs). Decode-
        // shaped MUL_MAT nodes on registered PQ tensors manage grouping
        // themselves inside the PQ hook.
        if (ggml_pq_enabled()) {
            ggml_pq_node_boundary(node, params.ith, params.nth, state->threadpool);
        }
        ...
        if (node_n + 1 < cgraph->n_nodes) {
            if (g_node_timing_on && state->ith == 0) {
                ...
                if (node->op == GGML_OP_MUL_MAT && graphs_seen_total > 8 && t_busy1 > t_busy0) {
                    ggml_pq_timing_note_mulmat(node->src[0]->name, t_busy1 - t_busy0);
                }
                const uint64_t b0 = ggml_time_us();
                ggml_barrier(state->threadpool);
                const uint64_t wait = ggml_time_us() - b0;
                g_node_timing.g_op_acc[node->op].barrier_us += wait;
                g_node_timing.barrier_total_us += wait;
            } else {
                ggml_barrier(state->threadpool);
            }
    ...
    if (g_node_timing_on && state->ith == 0) {
        ...
        if (++graphs_seen >= 32) {
            graphs_seen = 0;
            g_node_timing.graphs = 32;
            ggml_node_timing_dump();
        }
    }

    // PQ decode fusion: flush a group deferred by the final node (llama
    // graphs never end on a PQ mul_mat, this is defensive)
    if (ggml_pq_enabled()) {
        ggml_barrier(state->threadpool);
        ggml_pq_graph_end(params.ith, params.nth, state->threadpool);
    }
