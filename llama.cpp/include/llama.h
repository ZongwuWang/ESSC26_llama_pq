...
extern "C" {
    ...
    struct llama_model_params {
        ...
        // PQ decode-time GEMV (pq_cpu kernels): quantize linear weights at load
        // with a product quantizer and use PQ GEMV kernels during token
        // generation (n_tokens==1). Prefill keeps the original weights.
        bool pq_decode;       // enable PQ decode GEMV
        int32_t pq_ds;        // d_sub (sub-vector dims), typical 2
        int32_t pq_mode;      // 0 = auto (S1 for q/k/v/o/gate/up, S2 for down),
                              // 1 = S1 (input-dim partition), 2 = S2 (output-dim)
    };
