...
ggml_tensor * llm_graph_context::build_ffn(
   ...
   llm_ffn_gate_type   type_gate,
    ...
    // Fused gate/up weight (PQ graph-level fusion): the tensor named
    // "*.ffn_gate_up" holds rows [gate; up]; run ONE mul_mat and split the
    // output into views. Requires the parallel-gate silu path with no
    // biases/scales (the PQ-fused models have none).
    const bool fused_gu = up && gate && type_op == LLM_FFN_SILU &&
                          type_gate == LLM_FFN_PAR &&
                          strstr(up->name, ".ffn_gate_up") != nullptr;
    ggml_tensor * tmp;
    if (fused_gu) {
        ggml_tensor * ff = build_lora_mm(up, cur);
        cb(ff, "ffn_gate_up", il);
        const int64_t n_ff = ff->ne[0] / 2;
        cur = ggml_view_2d(ctx0, ff, n_ff, ff->ne[1],
                ggml_row_size(ff->type, ff->ne[0]), 0);
        cb(cur, "ffn_gate", il);
        tmp = ggml_view_2d(ctx0, ff, n_ff, ff->ne[1],
                ggml_row_size(ff->type, ff->ne[0]), ggml_row_size(ff->type, n_ff));
        cb(tmp, "ffn_up", il);
    } else {
