...
void llama_model_loader::done_getting_tensors(bool partial) const {
    // a GGUF with PQ side tensors (from llama-pq-convert) legitimately carries
    // extra tensors that are consumed by the PQ registry, not create_tensor;
    // allow them regardless of whether PQ decode is enabled
    if (!partial) {
        for (int i = 0; i < gguf_get_n_tensors(metadata); i++) {
            if (strstr(gguf_get_tensor_name(metadata, i), ".pq_meta")) {
                partial = true;
                break;
            }
        }
