...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // with --pq-decode the GGUF may carry extra side tensors (PQ codebooks/
    // indices from llama-pq-convert) that are consumed directly by the PQ
    // registry, not via create_tensor
    ml.done_getting_tensors(params.pq_decode);
    ...
    // PQ decode-time GEMV: quantize linear weights + register with ggml-cpu.
    // Decode (n_tokens==1) mul_mat calls on registered tensors then run the
    // PQ GEMV kernels; prefill keeps the original weights.
    if (params.pq_decode) {
        llama_pq_begin(params);
        const int64_t t0 = ggml_time_ms();
        int n_reg = 0;
        int pq_ds_actual = 0;
        static const bool no_qkv_fusion = getenv("GGML_PQ_NO_QKV") != nullptr;
        for (auto & layer : layers) {
            // Graph-level QKV fusion: materialize a fused weight (rows
            // [wq; wk; wv]) so the builder emits ONE mul_mat node, and run
            // the three members' PQ GEMVs as a single barrier pass. The
            // fused weights cost ~2x n_embd^2 bytes per layer of memory.
            ggml_tensor * fused_wqkv = nullptr;
            if (!no_qkv_fusion && layer.wq && layer.wk && layer.wv && !layer.wqkv) {
                int64_t qkv_offs[3];
                fused_wqkv = llama_pq_build_qkv_weight(layer.wq, layer.wk, layer.wv, qkv_offs);
                if (fused_wqkv) {
                    layer.wqkv = fused_wqkv;
                }
            }
            bool qkv_as_group = false;
            if (layer.wqkv) {
                if (llama_pq_register_qkv_group(ml, layer.wqkv->name,
                                                layer.wq, layer.wk, layer.wv, nullptr)) {
                    qkv_as_group = true;
                    n_reg++;
                } else {
                    layer.wqkv = nullptr;   // fall back to the separate path
                }
            }
            // Graph-level gate/up fusion: same pattern as the QKV fusion
            // (+2*n_ff*n_embd*type_size bytes per layer of fused weights).
            // layer.ffn_up is replaced by the fused tensor; build_ffn detects
            // it by name and splits the output into gate/up views.
            ggml_tensor * orig_up = layer.ffn_up;
            const char * gu_up_name = orig_up ? orig_up->name : nullptr;
            bool gu_as_group = false;
            if (!no_qkv_fusion && layer.ffn_gate && layer.ffn_up) {
                struct ggml_tensor * ts[2] = { layer.ffn_gate, layer.ffn_up };
                int64_t gu_offs[2];
                struct ggml_tensor * fused_gu = llama_pq_build_fused_weight(ts, 2,
                        ".ffn_gate.weight", ".ffn_gate_up", gu_offs);
                if (fused_gu) {
                    layer.ffn_up = fused_gu;
                    // register both members (gate: unchanged tensor; up: the
                    // original, registered under its pre-swap name)
                    int mds0 = 0, mds1 = 0;
                    const bool ok0 = llama_pq_register_from_loader(ml, layer.ffn_gate->name, &mds0) ||
                                     llama_pq_register_tensor(layer.ffn_gate, 0, 2);
                    const bool ok1 = llama_pq_register_from_loader(ml, gu_up_name, &mds1) ||
                                     (orig_up && llama_pq_register_tensor(orig_up, 0, 2));
                    const int64_t n_out = layer.ffn_gate->ne[1] + orig_up->ne[1];
                    const char * gu_names[2] = { layer.ffn_gate->name, gu_up_name };
                    if (ok0 && ok1 && mds0 == mds1 &&
                        ggml_pq_register_group(fused_gu->name, layer.ffn_gate->ne[0], n_out,
                                               gu_names, 2, gu_offs)) {
                        gu_as_group = true;
                        n_reg++;
                    } else {
                        layer.ffn_up = orig_up;   // fall back to the separate path
                    }
                }
            }
            struct { ggml_tensor * t; int mode_auto; } cands[] = {
                { qkv_as_group ? nullptr : layer.wq, 0 },
                { qkv_as_group ? nullptr : layer.wk, 0 },
                { qkv_as_group ? nullptr : layer.wv, 0 },
                { layer.wo,       0 },
                { gu_as_group ? nullptr : layer.ffn_gate, 0 },
                { gu_as_group ? nullptr : layer.ffn_up, 0 },
                { layer.ffn_down, 1 },
            };
            for (auto & c : cands) {
                if (!c.t) continue;
                // pre-computed side tensors in the GGUF (from pq-convert):
                // instant registration, no k-means at load
                if (llama_pq_register_from_loader(ml, c.t->name, &pq_ds_actual)) {
                    n_reg++;
                    continue;
                }
                int mode = c.mode_auto;
                if (params.pq_mode == 1) mode = 0;
                else if (params.pq_mode == 2) mode = 1;
                if (llama_pq_register_tensor(c.t, mode, params.pq_ds)) {
                    n_reg++;
                } else {
                    LLAMA_LOG_INFO("%s: PQ skipped tensor '%s'\n", __func__, c.t->name);
                }
            }
        }
        llama_pq_finish();
        LLAMA_LOG_INFO("%s: PQ decode enabled: %d tensors quantized (ds=%d, mode=%d) in %lld ms\n",
                       __func__, n_reg, pq_ds_actual ? pq_ds_actual : params.pq_ds,
                       params.pq_mode, (long long)(ggml_time_ms() - t0));
    }
