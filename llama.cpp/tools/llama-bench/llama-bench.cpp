...
struct cmd_params {
    ...
    std::vector<bool>                pq_decode;
    std::vector<int>                 pq_ds;
...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  -pd, --pq-decode <0|1>                      PQ decode-time GEMV (default: %s)\n", join(cmd_params_defaults.pq_decode, ",").c_str());
    printf("      --pq-ds <N>                             PQ sub-vector dims (default: %s)\n", join(cmd_params_defaults.pq_ds, ",").c_str());
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
            ...
            } else if (arg == "-pd" || arg == "--pq-decode") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
                }
                auto p = string_split<bool>(argv[i], split_delim);
                params.pq_decode.insert(params.pq_decode.end(), p.begin(), p.end());
            } else if (arg == "--pq-ds") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
                ...
                auto p = parse_int_range(argv[i]);
                params.pq_ds.insert(params.pq_ds.end(), p.begin(), p.end());
            } else if (arg == "--numa") {
                if (++i >= argc) {
                    invalid_param = true;
                    break;
    ...
    if (params.pq_decode.empty()) {
        params.pq_decode = cmd_params_defaults.pq_decode;
    }
    if (params.pq_ds.empty()) {
        params.pq_ds = cmd_params_defaults.pq_ds;
    }
...
struct cmd_params_instance {
    ...
    bool equal_mparams(const cmd_params_instance & other) const {
        return model == other.model && n_gpu_layers == other.n_gpu_layers && n_cpu_moe == other.n_cpu_moe &&
               ...
               pq_decode == other.pq_decode && pq_ds == other.pq_ds &&
...
static std::vector<cmd_params_instance> get_cmd_params_instances(const cmd_params & params) {
    ...
    for (const auto & pqd : params.pq_decode)
    for (const auto & pqds : params.pq_ds)
...
struct test {
    ...
    test(const cmd_params_instance & inst, const llama_model * lmodel, const llama_context * ctx) :
        ...
        pq             = inst.pq_decode ? "on/ds=" + std::to_string(inst.pq_ds) : "off";
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "no_op_offload",  "no_host",        "pq",             "fit_target",     "fit_min_ctx",
...
struct markdown_printer : public printer {
    ...
    void print_header(const cmd_params & params) override {
        ...
        if (params.pq_decode.size() > 1 || params.pq_decode != cmd_params_defaults.pq_decode ||
            params.pq_ds.size() > 1 || params.pq_ds != cmd_params_defaults.pq_ds) {
            fields.emplace_back("pq");
        }
